...
public class EndpointAPI {
	...
	public void addEndpoint(Exchange exchange) {
		...
		try {
			...
			exchange.getIn().setBody(
					new RestAnswer("Endpoint successfully added.", String
							.valueOf(endpointID)));
	...
	public void deleteEndpoint(Integer endpointID, Exchange exchange) {
		if (esa.deleteEndpoint(endpointID)) {
			exchange.getIn().setBody(
					new RestAnswer("Endpoint Successfully deleted", String
							.valueOf(endpointID)));
		} else {
			exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
			exchange.getIn().setBody(
					"Endpoint not found: " + String.valueOf(endpointID));
			exchange.getIn().setHeader(Exchange.HTTP_RESPONSE_CODE, 404);
		}
	}
	...
	public void updateEndpoint(Integer endpointID, Exchange exchange) {
		...
		try {
			if (esa.updateEndpoint(endpointID, endpoint.getSituations(),
				...
				exchange.getIn().setBody(
						new RestAnswer("Endpoint successfully updated", String
								.valueOf(endpointID)));
			} else {
				exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
				exchange.getIn().setBody(
	...
	public void addHandledSituation(Integer endpointId, Exchange exchange) {
		HandledSituation handledSituation = exchange.getIn().getBody(
				HandledSituation.class);

		try {
			int sitId = esa.addHandledSituation(endpointId, handledSituation);

			exchange.getIn().setBody(
					new RestAnswer("Situation successfully added.", String
							.valueOf(sitId)));
		} catch (InvalidEndpointException e) {
			exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
			exchange.getIn().setBody(e.getMessage());
	...
	public void getHandledSituation(Integer situationId, Exchange exchange) {
		HandledSituation handledSituation = esa
				.getHandledSituationById(situationId);
		if (handledSituation == null) {
			exchange.getIn().setBody(
					"Situation with id " + situationId + " not found.");
			exchange.getIn().setHeader(Exchange.HTTP_RESPONSE_CODE, 404);
			exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
		} else {
	...
	public void deleteHandledSituation(Integer situationId, Exchange exchange) {
		...
		if (esa.deleteHandledSituation(situationId)) {
			exchange.getIn().setBody(
					new RestAnswer("Situation Successfully deleted", String
							.valueOf(situationId)));
		} else {
			exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
			exchange.getIn().setBody(
					"Situation not found: " + String.valueOf(situationId));
			exchange.getIn().setHeader(Exchange.HTTP_RESPONSE_CODE, 404);
		}
	}
	...
	public void updateHandledSituation(Integer situationId, Exchange exchange) {
		...
		try {
			if (esa.updateHandledSituation(situationId, handledSituation)) {
				exchange.getIn().setBody(
						new RestAnswer("Situation successfully updated", String
								.valueOf(situationId)));
			} else {
				exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
				exchange.getIn().setBody(
						"situationId " + situationId + " not found.");
				exchange.getIn().setHeader(Exchange.HTTP_RESPONSE_CODE, 404);
			}
		} catch (InvalidEndpointException e) {
			exchange.getIn().setHeader(Exchange.CONTENT_TYPE, "text/plain");
